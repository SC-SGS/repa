 ...
 */
 bool Graph::repartition(const repart::Metric &m,
...
#ifdef GRAPH_DEBUG
  //std::cout << "Vtxdist: ";
  //std::copy(std::begin(vtxdist), std::end(vtxdist), std::ostream_iterator<idx_t>(std::cout, " "));
  //std::cout << std::endl;
#endif
  ...
  for (int i = 0; i < localCells; ++i) {
    ...
    // Edge weights must not be strictly positive. Add 1 and scale the true weight.
    // Also leave out the loop (i, i), i.e. start at n = 1.
    for (int n = 1; n < 27; ++n) {
      ...
      if (neigh < n_local_cells()) {
        // Local symmetry -- only ensures local symmetry, however, symmetry is
        // also required for cross-boundary edges.
    ...
    if (wsum > std::numeric_limits<idx_t>::max() / comm_cart.size()) {
      ...
      if (IDXTYPEWIDTH == 32)
        std::fprintf(stderr, "- Recompile a 64bit ParMETIS as this version only uses 32bit.");
      errexit();
    }
  }
...
#ifdef GRAPH_DEBUG
  //std::cout << "xadj: ";
  //std::copy(std::begin(xadj), std::end(xadj), std::ostream_iterator<idx_t>(std::cout, " "));
  //std::cout << std::endl;
#endif
  ...
  MPI_Allgatherv(part.data(), nvtx, MPI_IDX_T,
                 partition.data(), recvcount.data(), displ.data(), MPI_IDX_T,
                 comm_cart);
#ifdef GRAPH_DEBUG
