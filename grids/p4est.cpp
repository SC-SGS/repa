...
{
  MPI_Allreduce(MPI_IN_PLACE, nquads_per_proc.data(), n_nodes,
...
// Note: This is a global index on the Z-curve and not a local cell index to
// cells.
gidx pos_morton_idx(double pos[3], double cell_size[3], double inv_cell_size[3]) {
  for (int d = 0; d < 3; ++d) {
...
void P4estGrid::set_optimal_cellsize() {
  ...
  m_grid_level = impl::count_trailing_zeros(ncells[0] | ncells[1] | ncells[2]);
...
void P4estGrid::create_grid() {
  ...
  if (!m_repartstate.after_repart) {
    ...
    auto oldconn = std::move(m_p4est_conn);
    // Create p4est structures
    m_p4est_conn = std::unique_ptr<p4est_connectivity_t>(
        p8est_connectivity_new_brick(m_brick_size[0], m_brick_size[1], m_brick_size[2],
                                     PERIODIC(0), PERIODIC(1), PERIODIC(2)));
    m_p4est = std::unique_ptr<p4est_t>(
        p4est_new_ext(comm_cart, m_p4est_conn.get(), 0, m_grid_level, true, 0, NULL, NULL));
  }

  // Information about first quads of each node
  // Assemble this as early as possible as it is necessary for position_to_rank.
  // As soon as this information is ready, we can start migrating particles.
  m_node_first_cell_idx.resize(n_nodes + 1);
  for (int i = 0; i < n_nodes; ++i) {
    p4est_quadrant_t *q = &m_p4est->global_first_position[i];
    double xyz[3];
    p4est_qcoord_to_vertex(m_p4est_conn.get(), q->p.which_tree, q->x, q->y, q->z,
                           xyz);
  ...
  while (tmp < m_grid_size[0] || tmp < m_grid_size[1] || tmp < m_grid_size[2])
    tmp <<= 1;
  m_node_first_cell_idx[n_nodes] = tmp * tmp * tmp;

  if (m_repartstate.after_repart)
    m_repartstate.exchange_start_callback();

  auto p4est_ghost = std::unique_ptr<p4est_ghost_t>(
      p4est_ghost_new(m_p4est.get(), P8EST_CONNECT_CORNER));
  auto p4est_mesh = std::unique_ptr<p4est_mesh_t>(
      p4est_mesh_new_ext(m_p4est.get(), p4est_ghost.get(), 1, 1, 0, P8EST_CONNECT_CORNER));
  
  m_num_local_cells = m_p4est->local_num_quadrants;
  m_num_ghost_cells = p4est_ghost->ghosts.elem_count;
  int num_cells = m_num_local_cells + m_num_ghost_cells;

  std::unique_ptr<sc_array_t> ni = std::unique_ptr<sc_array_t>(sc_array_new(sizeof(int)));
  // Collect info about local cells
  m_p4est_shell.clear(); // Need to clear because we push_back
  m_p4est_shell.reserve(num_cells);
  for (int i = 0; i < m_num_local_cells; ++i) {
    p4est_quadrant_t *q = p4est_mesh_get_quadrant(m_p4est.get(), p4est_mesh.get(), i);
    double xyz[3];
    p4est_qcoord_to_vertex(m_p4est_conn.get(), p4est_mesh->quad_to_tree[i], q->x, q->y, q->z, xyz);

    int ql = 1 << p4est_tree_array_index(m_p4est->trees, p4est_mesh->quad_to_tree[i])->maxlevel;
    int x = xyz[0] * ql;
    int y = xyz[1] * ql;
    int z = xyz[2] * ql;
    ...
    m_p4est_shell.emplace_back(
        i, this_node, bndry? impl::CellType::boundary : impl::CellType::inner,
        bndry, x, y, z);

    // Neighborhood
    for (int n = 0; n < 26; ++n) {
      m_p4est_shell[i].neighbor[n] = -1;
      p4est_mesh_get_neighbors(m_p4est.get(), p4est_ghost.get(), p4est_mesh.get(), i, n, NULL, NULL, ni.get());
      ...
      m_p4est_shell[i].neighbor[n] = neighrank;

      if (neighrank >= m_p4est->local_num_quadrants) {
        // Ghost cell on inner subdomain boundaries
        m_p4est_shell[i].shell = impl::CellType::boundary;
      }
  ...
  for (int g = 0; g < m_num_ghost_cells; ++g) {
    ...
    double xyz[3];
    p4est_qcoord_to_vertex(m_p4est_conn.get(), q->p.piggy3.which_tree, q->x, q->y, q->z, xyz);

    int ql = 1 << p4est_tree_array_index(m_p4est->trees, q->p.piggy3.which_tree)->maxlevel;
    int x = xyz[0] * ql;
    int y = xyz[1] * ql;
    int z = xyz[2] * ql;

    m_p4est_shell.emplace_back(g, p4est_mesh->ghost_to_proc[g], impl::CellType::ghost, 0, x, y, z);
  }
}
...
void P4estGrid::prepare_communication() {
  ...
  std::vector<std::vector<lidx>> send_idx(n_nodes);
  std::vector<std::vector<gidx>> recv_idx(n_nodes);

  // Find all cells to be sent or received
  for (int i = 0; i < num_cells; ++i) {
    // Ghost cell? -> add to recv lists
    if (m_p4est_shell[i].shell == impl::CellType::ghost) {
      int nrank = m_p4est_shell[i].rank;
      if (nrank >= 0)
        recv_idx[nrank].push_back(i);
    }
    // Boundary cell? -> add to send lists
    if (m_p4est_shell[i].shell == impl::CellType::boundary) {
      // Add to all possible neighbors
      for (int n = 0; n < 26; ++n) {
        int nidx = m_p4est_shell[i].neighbor[n];
        // Invalid neighbor?
        if (nidx < 0 || m_p4est_shell[nidx].shell != impl::CellType::ghost)
          continue;

        int nrank = m_p4est_shell[nidx].rank;
        if (nrank < 0)
          continue;
  ...
  // Count the number of communications and assign indices for the communication
  int num_comm_proc = 0;
  std::vector<int> comm_proc(n_nodes, -1);
  for (int i = 0; i < n_nodes; ++i) {
    if (send_idx[i].size() != 0 && recv_idx[i].size() != 0)
      comm_proc[i] = num_comm_proc++;
    else if (!(send_idx[i].size() == 0 && recv_idx[i].size() == 0))
      throw std::runtime_error("Unexpected mismatch in send and receive lists.\n");
  }
...
lgidx P4estGrid::cell_neighbor_index(lidx cellidx, int neigh) {
  // Indices of the half shell neighbors in m_p4est_shell
  ...
  if (neigh == 0)
    ...
    return m_p4est_shell[cellidx].neighbor[hs_idxs[neigh]];
  else if (neigh >= 14 && neigh < 27)
    return m_p4est_shell[cellidx].neighbor[fs_idxs[neigh - hs_idxs.size()]];
  else
...
lidx P4estGrid::position_to_cell_index(double pos[3]) {
  auto shellidxcomp = [](const impl::LocalShell& s, int idx) {
    int64_t sidx = impl::cell_morton_idx(s.coord[0],
                                         s.coord[1],
                                         s.coord[2]);
    return sidx < idx;
  };

  auto needle = impl::pos_morton_idx(pos, m_cell_size.data(),
                                     m_inv_cell_size.data());

  auto shell_local_end = std::begin(m_p4est_shell) + n_local_cells();
  auto it = std::lower_bound(std::begin(m_p4est_shell),
                             // Only take into account local cells!
                             // This cannot be extended to ghost cells as these
                             // are not stored in SFC order in m_p4est_shell.
                             shell_local_end, needle, shellidxcomp);

  if (it != shell_local_end &&
      // Exclude finding cell 0 (lower_bound) if 0 is not the wanted result
      impl::cell_morton_idx(it->coord[0], it->coord[1], it->coord[2]) == needle)
    return std::distance(std::begin(m_p4est_shell), it);
  else
    throw std::domain_error("Pos not in local domain.");
}

rank P4estGrid::position_to_rank(double pos[3]) {
  // Cell of pos might not be known on this process (not in m_p4est_shell).
  ...
  auto it = std::upper_bound(std::begin(m_node_first_cell_idx),
                             ...
                             impl::pos_morton_idx(pos, m_cell_size.data(),
                                                  m_inv_cell_size.data()),
...
nidx P4estGrid::position_to_neighidx(double pos[3]) {
  ...
  auto it = std::lower_bound(std::begin(m_neighranks), std::end(m_neighranks), rank);
  if (*it != rank)
...
std::array<int, 3> P4estGrid::grid_size()
{
  return m_grid_size;
}
...
bool P4estGrid::repartition(const repart::Metric& m,
  ...
  double localsum = std::accumulate(std::begin(weights), std::end(weights), 0.0);
  ...
  std::accumulate(std::begin(weights), std::end(weights),
                  prefix,
  ...
  if (m_repartstate.nquads_per_proc[this_node] == 0) {
  ...
  // Reinitialize the grid and prepare its internal datastructures for querying
  // by generic_dd.
  m_repartstate.after_repart = true;
  m_repartstate.exchange_start_callback = exchange_start_callback;

  p4est_partition_given(m_p4est.get(), m_repartstate.nquads_per_proc.data());
  reinitialize();
