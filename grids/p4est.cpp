...
RepartState::RepartState(const boost::mpi::communicator& comm_cart): comm_cart(comm_cart), after_repart(false), nquads_per_proc(comm_cart.size()) {}
...
// Note: This is a global index on the Z-curve and not a local cell index to
// cells.
gidx pos_morton_idx(double box_l[3], double pos[3], double cell_size[3], double inv_cell_size[3]) {
  for (int d = 0; d < 3; ++d) {
...
void P4estGrid::create_grid() {
  ...
  if (!m_repartstate.after_repart) {
    ...
    m_p8est = std::unique_ptr<p8est_t>(
        p8est_new_ext(comm_cart, m_p8est_conn.get(), 0, m_grid_level, true, 0, NULL, NULL));
  }
  ...
  for (int i = 0; i < comm_cart.size(); ++i) {
    ...
    double xyz[3];
    p8est_qcoord_to_vertex(m_p8est_conn.get(), q->p.which_tree, q->x, q->y, q->z,
                           xyz);
  ...
  auto p8est_mesh = std::unique_ptr<p8est_mesh_t>(
      p8est_mesh_new_ext(m_p8est.get(), p8est_ghost.get(), 1, 1, 0, P8EST_CONNECT_CORNER));
  ...
  for (int i = 0; i < m_num_local_cells; ++i) {
    p8est_quadrant_t *q = p8est_mesh_get_quadrant(m_p8est.get(), p8est_mesh.get(), i);
    double xyz[3];
    p8est_qcoord_to_vertex(m_p8est_conn.get(), p8est_mesh->quad_to_tree[i], q->x, q->y, q->z, xyz);

    int ql = 1 << p8est_tree_array_index(m_p8est->trees, p8est_mesh->quad_to_tree[i])->maxlevel;
    int x = xyz[0] * ql;
    int y = xyz[1] * ql;
    int z = xyz[2] * ql;
    ...
    m_p8est_shell.emplace_back(
        i, comm_cart.rank(), bndry? impl::CellType::boundary : impl::CellType::inner,
        bndry, x, y, z);

    // Neighborhood
    for (int n = 0; n < 26; ++n) {
      m_p8est_shell[i].neighbor[n] = -1;
      p8est_mesh_get_neighbors(m_p8est.get(), p8est_ghost.get(), p8est_mesh.get(), i, n, NULL, NULL, ni.get());
  ...
  for (int g = 0; g < m_num_ghost_cells; ++g) {
    p8est_quadrant_t *q = p8est_quadrant_array_index(&p8est_ghost->ghosts, g);
    double xyz[3];
    p8est_qcoord_to_vertex(m_p8est_conn.get(), q->p.piggy3.which_tree, q->x, q->y, q->z, xyz);

    int ql = 1 << p8est_tree_array_index(m_p8est->trees, q->p.piggy3.which_tree)->maxlevel;
    int x = xyz[0] * ql;
    int y = xyz[1] * ql;
    int z = xyz[2] * ql;

    m_p8est_shell.emplace_back(g, p8est_mesh->ghost_to_proc[g], impl::CellType::ghost, 0, x, y, z);
  }
}
  ...
  for (int i = 0; i < num_cells; ++i) {
    ...
    if (m_p8est_shell[i].shell == impl::CellType::boundary) {
      ...
      for (int n = 0; n < 26; ++n) {
        ...
        // Invalid neighbor?
        if (nidx < 0 || m_p8est_shell[nidx].shell != impl::CellType::ghost)
          continue;
...
P4estGrid::P4estGrid(const boost::mpi::communicator& comm, Vec3d box_size, double min_cell_size):
...
lidx P4estGrid::position_to_cell_index(double pos[3]) {
  ...
  auto it = std::lower_bound(std::begin(m_p8est_shell),
...
bool P4estGrid::repartition(const repart::Metric& m,
  ...
  std::accumulate(std::begin(weights), std::end(weights),
                  ...
                  [this, target](double cellpref, double weight) {
                    int proc = std::min<int>(cellpref / target, comm_cart.size() - 1);
