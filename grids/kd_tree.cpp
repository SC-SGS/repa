...
int KDTreeGrid::linearize(const Vector3i& cell_position, const Vector3i& domain_size) 
{
  ...
  #ifndef NDEBUG
  for (int dim = 0; dim < 3; dim++) {
    assert(cell_position[dim] >= 0 && cell_position[dim] < domain_size[dim]);
  }
  #endif
...
void KDTreeGrid::init_nb_of_cells()
{
  ...
  m_nb_of_ghost_cells = volume(m_local_ghostdomain) - volume(m_local_subdomain);
}
...
std::vector<Domain> KDTreeGrid::intersection_domains(const Domain& localdomain,
    const Domain& ghostdomain, bool ghostdomain_coords,
  ...
  #ifndef NDEBUG
  {
    ...
    for (int dim = 0; dim < 3; dim++) {
      ...
      assert(ghostdomain.first[dim] >= m_global_ghostdomain.first[dim]
          && ghostdomain.first[dim] <= m_global_ghostdomain.second[dim]);
      assert(ghostdomain.second[dim] >= m_global_ghostdomain.first[dim]
          && ghostdomain.second[dim] <= m_global_ghostdomain.second[dim]);
    ...
    const Domain& neighbor_subdomain)
{
  ...
  for (const Vector3i& intersecting_cellvector : intersecting_cellvectors) {
    ...
    int lgidx = m_index_permutations[linearize(
      local_cellvector,
...
lgidx KDTreeGrid::cell_neighbor_index(lidx cellidx, int neigh)
{
  ...
  return m_index_permutations[linearize(
    cellvector, 
    m_local_ghostdomain_size
  )];
}
