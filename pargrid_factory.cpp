...
namespace impl {
ParallelLCGrid *make_pargrid_impl(GridType gt, const boost::mpi::communicator& comm, Vec3d box_size, double min_cell_size) {
  switch (gt) {
...
make_pargrid(GridType gt, const boost::mpi::communicator& comm, Vec3d box_size, double min_cell_size) {
  return std::unique_ptr<ParallelLCGrid>(impl::make_pargrid_impl(gt, comm, box_size, min_cell_size));
}
