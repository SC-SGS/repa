...
make_cartesian_communicator(const boost::mpi::communicator &comm)
{
  ...
  return boost::mpi::communicator{_comm_cart, boost::mpi::comm_take_ownership};
}

ParallelLCGrid::ParallelLCGrid(const boost::mpi::communicator &comm,
                               Vec3d box_size, double min_cell_size)
    : comm(comm, boost::mpi::comm_duplicate),
      comm_cart(make_cartesian_communicator(comm)), box_l(box_size),
      node_grid(util::mpi_cart_get_dims(comm_cart)),
      node_pos(util::mpi_cart_get_coords(comm_cart)), max_range(min_cell_size)
{
}
